...
export default function AdminApprovalsPage() {
  ...
  const handleUpdate = async (id: string, newStatus: string, name?: string) => {
    try {
      const token = localStorage.getItem('adminToken') || localStorage.getItem('studentToken');
      ...
      if (response.ok) {
        ...
        if (newStatus === 'APPROVED') {
           alert(`Account for ${name || 'User'} has been approved! They can now log in with their registered credentials.`);
        }
      } else {
                  ...
                  <td className="p-4 text-right">
                    {approval.status === 'PENDING' && (
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          onClick={() => handleUpdate(approval.id, 'APPROVED', approval.name)}
