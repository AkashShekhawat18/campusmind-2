        ...
        >
          ...
          {stats?.activeApprovals === 0 ? (
            ...
            <div className="space-y-4">
              {/* Dummy data for visual presentation since DB might be empty */}
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-start gap-4 p-4 rounded-xl hover:bg-white/5 transition-colors border border-white/5">
                  <div className="p-2 rounded-lg bg-blue-500/20 text-blue-400 mt-1">
                    <UserCircle size={20} />
                  </div>
                  <div>
                    <h4 className="text-white font-medium text-sm">New Student Registration</h4>
                    <p className="text-gray-400 text-xs mt-1">User ID: STU-88{i}9 registered successfully.</p>
                    <span className="text-xs text-gray-500 mt-2 block">{i * 2} hours ago</span>
                  </div>
                </div>
              ))}
            </div>
