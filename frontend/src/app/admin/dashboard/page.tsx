...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen relative flex flex-col p-6 pt-20">
      ...
      <nav className="fixed top-0 left-0 w-full glass-panel z-40 px-6 py-4 flex justify-between items-center border-b border-foreground/10">
        ...
        <div className="flex items-center gap-4">
          <span className="text-sm text-foreground/70">Welcome, Super Admin</span>
        ...
        <div className="lg:col-span-2 space-y-8">
            ...
            {stats.map((stat, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className={`glass-panel p-6 rounded-2xl border-l-4 ${stat.color}`}>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="p-2 rounded-lg bg-foreground/5">{stat.icon}</div>
                </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-panel rounded-2xl overflow-hidden glow-border">
            <div className="p-6 border-b border-foreground/10 flex justify-between items-center bg-foreground/5">
              <h3 className="text-xl font-bold text-foreground">User Approvals</h3>
              <div className="flex gap-2">
                <button 
                  onClick={() => setActiveTab('students')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'students' ? 'bg-neon-cyan text-black' : 'text-foreground/60 hover:text-foreground'}`}
                >
                  Students
                </button>
                <button 
                  onClick={() => setActiveTab('teachers')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'teachers' ? 'bg-electric-violet text-foreground' : 'text-foreground/60 hover:text-foreground'}`}
                >
                  Teachers
                </button>
              </div>
            ...
            <div className="p-0">
              <table className="w-full text-left text-sm text-foreground/70">
                <thead className="bg-foreground/5 text-foreground/60 uppercase text-xs">
                  <tr>
                ...
                <tbody className="divide-y divide-foreground/5">
                  {(activeTab === 'students' ? pendingStudents : pendingTeachers).map((user, i) => (
                    <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.1 }} key={user.id} className="hover:bg-foreground/5 transition-colors">
                      <td className="px-6 py-4 font-medium text-foreground">
                        {user.name}
                        <div className="text-xs text-foreground/50 font-normal">{user.date}</div>
                      </td>
                  ...
                  {(activeTab === 'students' ? pendingStudents : pendingTeachers).length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-6 py-8 text-center text-foreground/50">No pending approvals.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="space-y-8">
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="glass-panel rounded-2xl glow-border h-full max-h-[800px] flex flex-col">
            <div className="p-6 border-b border-foreground/10 bg-foreground/5">
              <h3 className="text-xl font-bold text-foreground">Activity Feed</h3>
            </div>
              ...
              <div className="relative border-l border-foreground/10 ml-3 space-y-8">
                {activityFeed.map((activity) => (
                  <div key={activity.id} className="relative pl-6">
                    ...
                    <div className="text-sm text-foreground/60">{activity.user}</div>
                    <div className="text-xs text-foreground/50 mt-1">{activity.time}</div>
                  </div>
                ))}
              </div>
            </div>
