  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("AI connection failed with status:", response.status, errText);
...
return (
  <AnimatePresence>
    <motion.div
      ...
      className={`fixed bottom-6 right-6 w-[400px] h-[600px] flex flex-col rounded-2xl border shadow-2xl z-50 overflow-hidden ${isDark ? 'bg-[#111113]/95 border-white/10 backdrop-blur-xl' : 'bg-white/95 border-black/10 backdrop-blur-xl'
        }`}
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm relative group ${msg.role === 'user'
