  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-6 py-24 z-20">
      <div className="text-center mb-16">
        <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-4 tracking-tight">Admin Control Center</h2>
        <p className="text-xl text-foreground font-bold max-w-2xl mx-auto">Complete oversight and access management without the clutter.</p>
      </div>
      ...
      <div className="w-full max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        {[
          ...
          <motion.div
            ...
            className="glass-panel p-8 rounded-3xl border border-foreground/5 flex flex-col gap-4 hover:bg-foreground/[0.04] transition-colors cursor-default"
          >
            <div className="w-12 h-12 rounded-xl bg-foreground/5 flex items-center justify-center mb-2 border border-foreground/10">
              <item.icon className="w-6 h-6 text-foreground" />
            </div>
            <h3 className="text-xl font-bold text-foreground">{item.title}</h3>
            <p className="text-foreground font-bold leading-relaxed">{item.desc}</p>
          </motion.div>
        ))}
      </div>
    </section>
